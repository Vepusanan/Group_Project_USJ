...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error("Invalid file type. Only JPG, PNG, GIF, or PDF are allowed."),
        false,
      );
    }
  },
...
export const handleMessageAttachment = (req, res, next) => {
  upload(req, res, async function (err) {
    if (err instanceof multer.MulterError) {
      return res
        .status(400)
        .json({ success: false, error: `File upload failed: ${err.message}` });
    } else if (err) {
    ...
    try {
      ...
      const publicUrl = await uploadMessageAttachmentBuffer(
        req.file.buffer,
        fileName,
        req.file.mimetype,
      );
